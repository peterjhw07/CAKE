...
def est_k_order(x, y, t0, half_life):
    k_guess = (((2 ** (x - 1)) - 1) * (r0 ** (1 - x)) * (y + 1)) / (((half_life - t0) ** (y + 1)) * (cat_add_rate ** y) * (x - 1))
    return k_guess

def est_k_first_order(y, t0, half_life):
...
if k_est == "" or k_est == 0:
    ...
    for it in range (0, len(k_guess)):
        if k_guess[it, 0] != 1:
            k_guess[it, 2] = est_k_order(k_guess[it, 0], k_guess[it, 1], t0_val, half_life)
        else:
            k_guess[it, 2] = est_k_first_order(k_guess[it, 1], t0_val, half_life)
        fit_guess = eq_sim_r(x_data, k_guess[it, 2], k_guess[it, 0], k_guess[it, 1], t0_val)
