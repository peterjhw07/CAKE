...
def residuals(y_data, fit, df):
    ...
    mean_error = np.sum(res) / len(res)
    rmse = (ss_res / len(res)) ** 0.5
    s_err = (ss_res / (len(res) - df)) ** 0.5  # for 1SD
    prop_s_err = s_err / np.mean(y_data)
    # chi_squared = np.sum(res / fit)
    AIC = (len(res) * math.log(2 * np.pi)) - (len(res) * math.log(len(res))) + len(res) \
        + (len(res) * math.log(ss_res)) + (2 * (df + 1))
    return [ss_res, r_squared, s_err]
...
def fit_cake(df, stoich_r, stoich_p, r0, p0, p_end, cat_add_rate, k_est, r_ord, cat_ord,
    ...
    if k_est is None or k_est == 0:
        ...
        for it in range(0, len(k_guess)):
            ...
            fit_guess = eq_sim_gen(stoich_r, stoich_p, r0, p0, cat_add_rate, x_data, k_guess[it, 2], k_guess[it, 0],
                                   k_guess[it, 1], t0_val)[0]  # need to change this to be suitable for r, p or rp and not just r
            _, k_guess[it, 3], _ = residuals(y_data, fit_guess, 2)
        print(y_data)
        print(k_guess)
    ...
    ss_res, r_squared, s_err = residuals(y_data, fit, df)
    ...
    #k, t0 = real_error_fit(x_data, y_data, k_val, 2, 2, t0_val, k_val, t0_val, k_min, t0_min, k_max, t0_max)
    #print(k, t0)

    real_err_run = "N"
    if "Y" in real_err_run:
        real_err_inc = 20  # enter number of increments between 0 and 3 orders
        real_err_inc += 1
        real_err_calc = np.zeros([real_err_inc * real_err_inc, 7])
        real_err_calc[:] = [[x, y, 0, 0, 0, 0, 0] for x in np.linspace(0, 2, real_err_inc) for y in np.linspace(0, 2, real_err_inc)]
        if len(t0_est) > 1:
            df_err = 2
        else:
            df_err = 1
        for it in range(0, len(real_err_calc)):
            x, y = real_err_calc[it, [0, 1]]
            res_err_fit = optimize.curve_fit(lambda x_data, k, t0: eq_sim_p(x_data, k, x, y, t0), x_data, y_data,
                                         [k_val, t0_val], maxfev=10000, bounds=((k_min, t0_min), (k_max, t0_max)))  # need to adapt to r, p or rp
            res_val_err_fit = res_err_fit[0]
            fit_real_err = eq_sim_p(x_data, res_val_err_fit[0], x, y, res_val_err_fit[1])  # need to adapt to r, p or rp
            real_err_calc[it, 2], real_err_calc[it, 3] = res_val_err_fit
            res_err_fit_err = np.sqrt(np.diag(res_err_fit[1]))  # for 1SD
            [ss_res_err_fit, r_squared_err_fit, s_squared_err_err_fit] = residuals(y_data, fit_real_err, df_err)
            real_err_calc[it, 4], real_err_calc[it, 5], real_err_calc[it, 6] = ss_res_err_fit, r_squared_err_fit, s_squared_err_err_fit
            real_err_calc_sort = real_err_calc[real_err_calc[:, 5].argsort()[::-1]]
        # print(real_err_calc_sort)
        real_err_calc_sort_cutoff = real_err_calc_sort  # [real_err_calc_sort[:, 5] > 0]
        # print(real_err_calc_sort_cutoff)
        export_err = "Y"
        if "Y" in export_err:
            exportdf = pd.DataFrame({"x": real_err_calc_sort_cutoff[:, 0], "y": real_err_calc_sort_cutoff[:, 1],
                                     "k": real_err_calc_sort_cutoff[:, 2],"t0": real_err_calc_sort_cutoff[:, 3],
                                     "ss_res": real_err_calc_sort_cutoff[:, 4], "r^2": real_err_calc_sort_cutoff[:, 5],
                                     "s": real_err_calc_sort_cutoff[:, 6]})
            with pd.ExcelWriter(
                    r'C:\Users\Peter\Documents\Postdoctorate\Work\CAKE\Programmes\Test_Spectra_Error_Results.xlsx',
                    mode='a', if_sheet_exists='replace') as writer:
                exportdf.to_excel(writer, sheet_name="4.2", index=False)

        cont_x_org, cont_y_org, cont_z_org = real_err_calc_sort_cutoff[:, 0], real_err_calc_sort_cutoff[:, 1], real_err_calc_sort_cutoff[:, 5]
        # cont_x_add, cont_y_add = cont_x_org, cont_y_org
        cont_x_add, cont_y_add = np.linspace(cont_x_org.min(), cont_x_org.max(), 1000), np.linspace(cont_y_org.min(), cont_y_org.max(), 1000)
        cont_x_plot, cont_y_plot = np.meshgrid(cont_x_add, cont_y_add)
        cont_z_plot = scipy.interpolate.griddata((cont_x_org, cont_y_org), cont_z_org, (cont_x_plot, cont_y_plot), method='linear')
        # rbf = scipy.interpolate.Rbf(cont_x_org, cont_y_org, cont_z_org, function='linear')
        # cont_z_plot = rbf(cont_x_plot, cont_y_plot)

        plt.imshow(cont_z_plot, vmin=cont_z_org.min(), vmax=cont_z_org.max(), origin='lower', cmap='coolwarm',
               extent=[cont_x_org.min(), cont_x_org.max(), cont_y_org.min(), cont_y_org.max()], aspect='auto')
        ...
        ax.plot_surface(cont_x_plot, cont_y_plot, cont_z_plot, cmap='coolwarm')  # rstride=1, cstride=1
        ax.set_xlabel('x')
        ax.set_ylabel('y')
        ax.set_zlabel('z')
        plt.show()
