...
def eq_sim_gen(r0, p0, cat_add_rate, t_fit, k, x, y, t0):
...
def eq_sim_r(r0, p0, cat_add_rate, t, k, x, y, t0):
    t_fit = t
    [r_calc, p_calc, rate_it] = eq_sim_gen(r0, p0, cat_add_rate, t_fit, k, x, y, t0)
    return r_calc

def eq_sim_p(r0, p0, cat_add_rate, t, k, x, y, t0):
    t_fit = t
    [r_calc, p_calc, rate_it] = eq_sim_gen(r0, p0, cat_add_rate, t_fit, k, x, y, t0)
    return p_calc

def eq_sim_rate(r0, p0, cat_add_rate, t, k, x, y, t0):
    t_fit = t
    [r_calc, p_calc, rate_it] = eq_sim_gen(r0, p0, cat_add_rate, t_fit, k, x, y, t0)
    return rate_it

def eq_sim_multi(r0, p0, cat_add_rate, t, k, x, y, t0):
    t_fit = t[:int(len(t) / 2)]
    [r_calc, p_calc, rate_it] = eq_sim_gen(r0, p0, cat_add_rate, t_fit, k, x, y, t0)
...
def eq_sim_r_inc(r0, p0, cat_add_rate, inc_adj, t, k, x, y, t0):
    t_fit = add_sim_t(t, inc_adj)
    [r_calc, p_calc, rate_it] = eq_sim_gen(r0, p0, cat_add_rate, t_fit, k, x, y, t0)
    r_calc_inc = find_exp_t(t, t_fit, r_calc)
    return r_calc_inc

def eq_sim_p_inc(r0, p0, cat_add_rate, inc_adj, t, k, x, y, t0):
    t_fit = add_sim_t(t, inc_adj)
    [r_calc, p_calc, rate_it] = eq_sim_gen(r0, p0, cat_add_rate, t_fit, k, x, y, t0)
    p_calc_inc = find_exp_t(t, t_fit, p_calc)
    return p_calc_inc

def eq_sim_rate_inc(r0, p0, cat_add_rate, inc_adj, t, k, x, y, t0):
    t_fit = add_sim_t(t, inc_adj)
    [r_calc, p_calc, rate_it] = eq_sim_gen(r0, p0, cat_add_rate, t_fit, k, x, y, t0)
    rate_it_inc = find_exp_t(t, t_fit, rate_it)
    return rate_it_inc

def eq_sim_multi_inc(r0, p0, cat_add_rate, inc_adj, t, k, x, y, t0):
...
def est_k_order(x, y, t0, cat_add_rate, half_life):
...
def CAKE_overall(stoich_r, stoich_p, r0, p0, p_end, react_vol_init, cat_sol_conc, inject_rate, win,
    ...
    if k_est == "" or k_est == 0:
        ...
        for it in range (0, len(k_guess)):
            if k_guess[it, 0] != 1:
                k_guess[it, 2] = est_k_order(k_guess[it, 0], k_guess[it, 1], t0_val, cat_add_rate, half_life)
            else:
    ...
    if fit_asp == 'r':
        if inc == 0:
            res = optimize.curve_fit(lambda t, k, x, y, t0: eq_sim_r(r0, p0, cat_add_rate, t, k, x, y, t0),
                                     x_data, y_data, init_param, maxfev=10000,
                                     bounds=((k_min, r_min, cat_min, t0_min),
                                             (k_max, r_max, cat_max, t0_max)))
        else:
            res = optimize.curve_fit(lambda t, k, x, y, t0: eq_sim_r_inc(r0, p0, cat_add_rate, inc_adj, t, k, x, y, t0),
                                     x_data, y_data, init_param, maxfev=10000,
                                     bounds=((k_min, r_min, cat_min, t0_min),
                                             (k_max, r_max, cat_max, t0_max)))
        kf, xf, yf, t0f = res[0]
        if p_col == "":
            fit = eq_sim_r(r0, p0, cat_add_rate, x_data, kf, xf, yf, t0f)
            fit_r = fit
        else:
            x_data = np.append(t, t)
            y_data = np.append(r, p)
            if inc == 0:
                fit = eq_sim_multi(r0, p0, cat_add_rate, x_data, kf, xf, yf, t0f)
            else:
                fit = eq_sim_multi_inc(r0, p0, cat_add_rate, inc_adj, x_data, kf, xf, yf, t0f)
            fit_r = fit[:int(len(x_data) / 2)]
            fit_p = fit[int(len(x_data) / 2):]
    elif fit_asp == 'p':
        if inc == 0:
            res = optimize.curve_fit(lambda t, k, x, y, t0: eq_sim_p(r0, p0, cat_add_rate, t, k, x, y, t0),
                                     x_data, y_data, init_param, maxfev=10000,
                                     bounds=((k_min, r_min, cat_min, t0_min),
                                             (k_max, r_max, cat_max, t0_max)))
        else:
            res = optimize.curve_fit(lambda t, k, x, y, t0: eq_sim_p_inc(r0, p0, cat_add_rate, inc_adj, t, k, x, y, t0),
                                     x_data, y_data, init_param, maxfev=10000,
                                     bounds=((k_min, r_min, cat_min, t0_min),
                                             (k_max, r_max, cat_max, t0_max)))
        kf, xf, yf, t0f = res[0]
        if r_col == "":
            if inc == 0:
                fit = eq_sim_p(r0, p0, cat_add_rate, x_data, kf, xf, yf, t0f)
            else:
                fit = eq_sim_p_inc(r0, p0, cat_add_rate, inc_adj, x_data, kf, xf, yf, t0f)
            fit_p = fit
        else:
            x_data = np.append(t, t)
            y_data = np.append(r, p)
        ...
        if inc == 0:
            res = optimize.curve_fit(lambda t, k, x, y, t0: eq_sim_multi(r0, p0, cat_add_rate, t, k, x, y, t0),
                                     x_data, y_data, init_param, maxfev=10000,
                                     bounds=((k_min, r_min, cat_min, t0_min),
                                             (k_max, r_max, cat_max, t0_max)))
            kf, xf, yf, t0f = res[0]
            ...
            res = optimize.curve_fit(lambda t, k, x, y, t0: eq_sim_multi_inc(r0, p0, cat_add_rate, inc_adj, t, k, x, y, t0),
                                     x_data, y_data, init_param, maxfev=10000,
                                     bounds=((k_min, r_min, cat_min, t0_min),
                                             (k_max, r_max, cat_max, t0_max)))
            kf, xf, yf, t0f = res[0]
        ...
        fit_p = fit[int(len(x_data) / 2):]
    if inc == 0:
        fit_rate = eq_sim_rate(r0, p0, cat_add_rate, x_data, kf, xf, yf, t0f)
    else:
        fit_rate = eq_sim_rate_inc(r0, p0, cat_add_rate, inc_adj, x_data, kf, xf, yf, t0f)
