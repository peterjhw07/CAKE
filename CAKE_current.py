...
if manual == "Y":
    ...
    inc = 1  # enter additional increments between adjacent points for improved simulation, i.e., 1 for using raw time points
...
def eq_sim_gen(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, t_fit, k, x, y, t0):
...
def eq_sim_r(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, t, k, x, y, t0):
    t_fit = t
    [r_calc, p_calc, rate_it] = eq_sim_gen(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, t_fit, k, x, y, t0)
    return r_calc

def eq_sim_p(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, t, k, x, y, t0):
    t_fit = t
    [r_calc, p_calc, rate_it] = eq_sim_gen(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, t_fit, k, x, y, t0)
    return p_calc

def eq_sim_rate(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, t, k, x, y, t0):
    t_fit = t
    [r_calc, p_calc, rate_it] = eq_sim_gen(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, t_fit, k, x, y, t0)
    return rate_it

def eq_sim_multi(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, t, k, x, y, t0):
    t_fit = t[:int(len(t) / 2)]
    [r_calc, p_calc, rate_it] = eq_sim_gen(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, t_fit, k, x, y, t0)
...
def add_sim_t(t, inc_adj):
    ...
    for it in range(0, len(t) - 1):
        new_t_it = np.linspace(t[it], t[it + 1], inc_adj)[:-1]
        t_fit[it * len(new_t_it):(it * len(new_t_it)) + len(new_t_it)] = new_t_it
    t_fit[-1] = t[-1]
    return t_fit

# find the location of original t values
def find_exp_t(t, t_fit, out, inc_adj):
    out_calc_inc = out[list(range(0, len(t_fit), inc_adj - 1))]
    return out_calc_inc

# equivalent to previous simulation functions but with more increments (x4)
def eq_sim_r_inc(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, inc_adj, t, k, x, y, t0):
    t_fit = add_sim_t(t, inc_adj)
    [r_calc, p_calc, rate_it] = eq_sim_gen(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, t_fit, k, x, y, t0)
    r_calc_inc = find_exp_t(t, t_fit, r_calc, inc_adj)
    return r_calc_inc

def eq_sim_p_inc(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, inc_adj, t, k, x, y, t0):
    t_fit = add_sim_t(t, inc_adj)
    [r_calc, p_calc, rate_it] = eq_sim_gen(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, t_fit, k, x, y, t0)
    p_calc_inc = find_exp_t(t, t_fit, p_calc, inc_adj)
    return p_calc_inc

def eq_sim_rate_inc(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, inc_adj, t, k, x, y, t0):
    t_fit = add_sim_t(t, inc_adj)
    [r_calc, p_calc, rate_it] = eq_sim_gen(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, t_fit, k, x, y, t0)
    rate_it_inc = find_exp_t(t, t_fit, rate_it, inc_adj)
    return rate_it_inc

def eq_sim_multi_inc(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, inc_adj, t, k, x, y, t0):
    t_fit = add_sim_t(t[:int(len(t) / 2)], inc_adj)
    [r_calc, p_calc, rate_it] = eq_sim_gen(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, t_fit, k, x, y, t0)
    r_calc_inc = find_exp_t(t[:int(len(t) / 2)], t_fit, r_calc, inc_adj)
    p_calc_inc = find_exp_t(t[:int(len(t) / 2)], t_fit, p_calc, inc_adj)
...
def est_k_first_order(y, t0, cat_add_rate, half_life):
    k_guess = ((y + 1) * math.log(2)) / (((half_life - t0) ** (y + 1)) * (cat_add_rate ** y))
    return k_guess

# smooth data (if required)
def data_manip(d_col, df, win):
...
def TIC_manip(data, TIC, TIC_col):
    if TIC_col != '""':
        data = data / TIC
    else:
        data = data
...
def CAKE_overall(stoich_r, stoich_p, r0, p0, p_end, react_vol_init, cat_sol_conc, inject_rate, win,
    ...
    t = data_manip(t_col, df, win)
    if TIC_col != '""':
        TIC = data_manip(TIC_col, df, win)
    else:
        TIC = 1
    if r_col != '""':
        r = data_manip(r_col, df, win)
        r = TIC_manip(r, TIC, TIC_col)
        if r0 == '""':
            ...
            r = r / r_scale
    if p_col != '""':
        p = data_manip(p_col, df, win)
        p = TIC_manip(p, TIC, TIC_col)
        if p_end == '""':
    ...
    if k_est == '""' or k_est == 0:
        ...
        for it in range (0, len(k_guess)):
            ...
            fit_guess = eq_sim_r(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, x_data, k_guess[it, 2], k_guess[it, 0], k_guess[it, 1], t0_val)
    ...
    if fit_asp == 'r':
        if inc == 0:
            res = optimize.curve_fit(lambda t, k, x, y, t0: eq_sim_r(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, t, k, x, y, t0),
                                     x_data, y_data, init_param, maxfev=10000,
                                     bounds=((k_min, r_min, cat_min, t0_min),
                                             (k_max, r_max, cat_max, t0_max)))
        else:
            res = optimize.curve_fit(lambda t, k, x, y, t0: eq_sim_r_inc(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, inc_adj, t, k, x, y, t0),
                                     x_data, y_data, init_param, maxfev=10000,
                                     bounds=((k_min, r_min, cat_min, t0_min),
                                             (k_max, r_max, cat_max, t0_max)))
        kf, xf, yf, t0f = res[0]
        if p_col == '""':
            fit = eq_sim_r(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, x_data, kf, xf, yf, t0f)
            fit_r = fit
        else:
            x_data = np.append(t, t)
            y_data = np.append(r, p)
            if inc == 0:
                fit = eq_sim_multi(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, x_data, kf, xf, yf, t0f)
            else:
                fit = eq_sim_multi_inc(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, inc_adj, x_data, kf, xf, yf, t0f)
            fit_r = fit[:int(len(x_data) / 2)]
            fit_p = fit[int(len(x_data) / 2):]
    elif fit_asp == 'p':
        if inc == 0:
            res = optimize.curve_fit(lambda t, k, x, y, t0: eq_sim_p(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, t, k, x, y, t0),
                                     x_data, y_data, init_param, maxfev=10000,
                                     bounds=((k_min, r_min, cat_min, t0_min),
                                             (k_max, r_max, cat_max, t0_max)))
        else:
            res = optimize.curve_fit(lambda t, k, x, y, t0: eq_sim_p_inc(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, inc_adj, t, k, x, y, t0),
                                     x_data, y_data, init_param, maxfev=10000,
                                     bounds=((k_min, r_min, cat_min, t0_min),
                                             (k_max, r_max, cat_max, t0_max)))
        kf, xf, yf, t0f = res[0]
        if r_col == '""':
            if inc == 0:
                fit = eq_sim_p(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, x_data, kf, xf, yf, t0f)
            else:
                fit = eq_sim_p_inc(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, inc_adj, x_data, kf, xf, yf, t0f)
            fit_p = fit
        else:
            x_data = np.append(t, t)
            y_data = np.append(r, p)
        ...
        if inc == 0:
            res = optimize.curve_fit(lambda t, k, x, y, t0: eq_sim_multi(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, t, k, x, y, t0),
                                     x_data, y_data, init_param, maxfev=10000,
                                     bounds=((k_min, r_min, cat_min, t0_min),
                                             (k_max, r_max, cat_max, t0_max)))
            kf, xf, yf, t0f = res[0]
            ...
            res = optimize.curve_fit(lambda t, k, x, y, t0: eq_sim_multi_inc(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, inc_adj, t, k, x, y, t0),
                                     x_data, y_data, init_param, maxfev=10000,
                                     bounds=((k_min, r_min, cat_min, t0_min),
                                             (k_max, r_max, cat_max, t0_max)))
            kf, xf, yf, t0f = res[0]
        ...
        fit_p = fit[int(len(x_data) / 2):]
    if inc == 0:
        fit_rate = eq_sim_rate(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, x_data, kf, xf, yf, t0f)
    else:
        fit_rate = eq_sim_rate_inc(stoich_r, stoich_p, r0, p0, react_vol_init, inject_rate, cat_add_rate, inc_adj, x_data, kf, xf, yf, t0f)
    ...
    if len(t0_est) > 1:
        ...
        #print("Catalyst poisoning and error")
        #print([cat_pois, cat_pois_err])
    else:
        cat_pois = 0
        cat_pois_err = 0
    ...
    return [k_val, res_val[0], res_err[0], res_val[1], res_err[1], res_val[2], res_err[2],
            res_val[3], res_err[3], cat_pois, cat_pois_err, ss_res, r_squared]
